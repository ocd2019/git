...
unsigned long LOG_PrintLogTime(unsigned char *ucTime, unsigned long ulBufLen)
{
    ...
    struct timeval stTmMsec;
	 ...
	 if(NULL == ucTime)	return LOG_FAILURE;
...
{
	 if(NULL == pucTypeString)	return LOG_FAILURE;
	 ...
	 {
	 	case LOG_DEBUG:
		   strncpy((char *)pucTypeString, "DEBUG", ulBufLen);break;
		case LOG_ERROR:
		   strncpy((char *)pucTypeString, "ERROR", ulBufLen);break;
		case LOG_WARNING:
		   strncpy((char *)pucTypeString, "WARNING", ulBufLen);break;
		case LOG_ACTION:
		   strncpy((char *)pucTypeString, "ACTION", ulBufLen);break;
		case LOG_SYSTEM:
		   strncpy((char *)pucTypeString, "SYSTEM", ulBufLen);break;
		default:
		   strncpy((char *)pucTypeString, "UNKNOWN", ulBufLen);break;
	 }
...
unsigned long LOG_OpenLogFile(void)
{
	 ...
	 /*判断文件是否已经打开*/
	 if(NULL != pFile) return LOG_SUCCESS;
	 
	 /*判断文件名是否有定义*/
	 if(NULL == path) return LOG_FAILURE;
	 
	 /*判断文件是否存在*/
	 if (!access(path, 0))
	 {
		  /*获取文件大小*/
		  if (0 > (len = get_file_size(path)))	return LOG_FAILURE;
	 }
	 ...
	 if(NULL == pFile) return LOG_FAILURE;
...
unsigned long LOG_PrintLog(unsigned char ucType, unsigned char *pucLogInfo)
{
	 ...
	 if(NULL == pucLogInfo)	return LOG_FAILURE;
	 ...
	 if(0 != ulResult)	return LOG_FAILURE;

	 snprintf((char *)ucLogInfo, sizeof(ucLogInfo) - 1, "[%s] [%s] %s", ucTime, ucLogTypeStr, pucLogInfo);

	 /*判断是否打印调试日志*/
	 if(PRINT_LOG_TO_TERM == g_ulPrintLogPlaceFlag)	return LOG_SUCCESS;
...
unsigned long LOG_Init(const unsigned char* ucLogFileName, unsigned long ulFileSize)
{
	 unsigned int i = 0;
	 /*判断参数的合法性*/
	 if((NULL == ucLogFileName) || !(ulFileSize > 0))	return LOG_FAILURE;

	 /*判断是否将日志输出到日志文件*/
	 if((PRINT_LOG_TO_FILE != g_ulPrintLogPlaceFlag) || (0 != g_ulLogInitFlag))	return LOG_SUCCESS;
	 
	 /*记录日志模块已经被初始化(防止改模块被重复初始化)*/
	 g_ulLogInitFlag = 1;
	 
	 /*生成存储日志的文件名*/
	 for(i = 0; i < NUMBER(g_ucLogFileName); i++)	snprintf((char *)g_ucLogFileName[i], sizeof(g_ucLogFileName[i]) - 1, "%s_%02d", ucLogFileName, i);
